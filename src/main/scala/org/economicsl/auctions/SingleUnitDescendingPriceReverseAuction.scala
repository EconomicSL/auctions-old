...
/** Class defining a single-unit descending price reverse auction.
  *
  ...
  */
class SingleUnitDescendingPriceReverseAuction[B <: LimitBidOrder with SingleUnit, A <: LimitAskOrder with Persistent with SingleUnit]
  ...
  final def fill(order: B): Option[Fill[A, B]] = findMatchFor(order, orderBook) map {
    case (_, askOrder) =>
...
object SingleUnitDescendingPriceReverseAuction {
  ...
  def apply[B <: LimitBidOrder with SingleUnit, A <: LimitAskOrder with Persistent with SingleUnit]
    ...
    new SingleUnitDescendingPriceReverseAuction[B, A](matchingRule, pricingRule, tradable)(ordering)
  }
