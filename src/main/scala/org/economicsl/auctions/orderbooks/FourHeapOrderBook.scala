...
object FourHeapOrderBook {
  ...
  def apply(): FourHeapOrderBook = {
    ...
    val matchedAskOrders = immutable.TreeSet.empty[persistent.LimitAskOrder with SingleUnit](LimitAskOrder.priority)

    /* Note that the heap priority is minimal price: lowest priced order should be at the top of the heap! */
    val matchedBidOrders = immutable.TreeSet.empty[persistent.LimitBidOrder with SingleUnit](LimitBidOrder.priority)

    val unMatchedAskOrders = immutable.TreeSet.empty[persistent.LimitAskOrder with SingleUnit]
    val unMatchedBidOrders = immutable.TreeSet.empty[persistent.LimitBidOrder with SingleUnit]
